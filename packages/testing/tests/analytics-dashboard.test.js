...
[false, true].forEach((embedded) => {
  screenModes.forEach((screenMode) => {
    ...
    test(`Analytics Dashboard (${project}, embed=${embedded}, ${screenMode[0]})`, async (t) => {
      ...
      await takeScreenshot(`analytics-dashboard-all-embed=${embedded}-${theme}-${screenMode[0]}`, 'body');
      await checkScreenMode(t, screenMode);
      await t.click(Selector('.dx-tabs .dx-item').nth(3));
      await t.wait(timeoutSecond);
      await takeScreenshot(`analytics-dashboard-year-embed=${embedded}-${theme}-${screenMode[0]}`, 'body');
