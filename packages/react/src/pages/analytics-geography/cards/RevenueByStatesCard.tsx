...
import VectorMap, { Layer, Legend, Source, Tooltip } from 'devextreme-react/vector-map';
...
export const RevenueByStatesCard = ({ datasource, mapsData }) => (
  <CardAnalytics title='Revenue by States' contentClass='sales-by-state-map' isLoading={!datasource}>
    <VectorMap id='vector-map' bounds={[-118, 50, -80, 25]}>
      <Layer dataSource={mapsData.usa} hoverEnabled={false} />
      <Layer dataSource={datasource} minSize={20} maxSize={40} sizeGroups={[0, 80000, 100000, 400000, 500000]} opacity={0.8} name='bubbles' elementType='bubble' dataField='value' />
      <Tooltip enabled customizeTooltip={customizeTooltip} />
      <Legend markerShape='circle' customizeItems={customizeItems} customizeText={customizeLegendText}>
